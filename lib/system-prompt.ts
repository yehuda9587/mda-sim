...
export default function MdaSimulator() {
  ...
  const startScenario = async () => {
    ...
    // שליחת בקשה סמויה להתחלת המקרה לאחר ההוראות
    triggerFirstCase(instructions);
  };

  const triggerFirstCase = async (instr: string) => {
        ...
        <div className="p-4 border-t border-slate-200 bg-white">
          <div className="flex gap-2">
            ...
            <button 
              ...
              disabled={isPaused || !isActive}
              className="bg-slate-900 text-white px-8 py-2 rounded-xl font-bold hover:bg-slate-800 shadow-md disabled:bg-slate-300 transition-all"
            >
              שלח
            </button>
          </div>
