    ...
    for (const modelInfo of MODELS) {
      try {
        const model = genAI.getGenerativeModel({ 
          ...
          generationConfig: { 
            temperature: 0, // נעילה מוחלטת למניעת שינויי פרטים
            topP: 0.1,
            maxOutputTokens: 150 // הגבלה למניעת חפירות מיותרות
          } as any
        });
        
        // בניית היסטוריה נקייה - מחליף 'assistant' ל-'model' עבור Gemini
        ...
        console.warn(`[MDA-SIM] Fallback from ${modelInfo.name}`);
        continue;
      }
    }

    if (!streamResult) throw new Error("AI Failure");
