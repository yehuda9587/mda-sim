...
export default function MdaSimulator() {
  ...
  const startScenario = async () => {
    ...
    // הזנקה שקטה ל-API כדי לקבל הוראות + מקרה בבת אחת
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans p-4 flex flex-col items-center">
      <div className="w-full max-w-2xl mb-4 flex gap-3 overflow-x-auto py-2 no-scrollbar">
        {history.map((h, i) => (
          <div key={i} className="bg-white border-b-4 border-b-blue-600 border border-slate-200 p-3 rounded-xl shadow-sm min-w-[100px] text-center transition-all hover:scale-105">
      ...
      <div className="w-full max-w-2xl bg-white shadow-sm border border-slate-200 rounded-2xl p-6 mb-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className={`text-xl font-mono font-bold px-4 py-1 rounded-full border shadow-sm transition-colors ${isPaused ? 'bg-orange-50 border-orange-200 text-orange-600 animate-pulse' : 'bg-blue-50 border-blue-100 text-blue-700'}`}>
              {formatTime(seconds)}
            </div>
          </div>
        </div>
