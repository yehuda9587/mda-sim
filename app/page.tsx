    ...
    <div className="h-dynamic w-full flex flex-col bg-slate-950 overflow-hidden relative">
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar bg-slate-950">
        <div className="max-w-2xl mx-auto w-full flex flex-col space-y-4">
          {messages.length === 0 && (
            <div className="h-[50vh] flex flex-col items-center justify-center text-slate-600 text-center opacity-40">
              <span className="text-6xl mb-4">🚑</span>
              <p className="text-lg">מוכן לסימולציה?<br/>הבוחן ימתין לפעולה הראשונה שלך.</p>
            </div>
          )}
          {messages.map((m, i) => (
          ...
          {!isActive ? (
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white px-5 rounded-xl font-bold transition-all active:scale-95"
              >
                שלח
              </button>
            </div>
